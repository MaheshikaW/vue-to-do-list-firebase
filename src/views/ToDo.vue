...
<table class="table table-bordered mt-5">
  ...
  <tbody>
    <tr v-for="(task,id) in tasks" :key="id">
      ...
      <td><div @click="editTask(task.id)"><span class="fa fa-pen text-center"></span></div></td>
      <td><div @click="deleteTask(task.id)"><span class="fa fa-trash text-center"></span></div></td>
    </tr>
  ...
  methods:{
   async submitTask(){
     ...
     if(this.taskEdit === null){
      await db.collection('tasks').add({name:this.task,status:this.availableStatuses[0]});
     }else{
       db.collection('tasks').doc(this.taskEdit).update({name:this.task})
     }
     this.task='';
     this.taskEdit=null;
   },
  ...
  editTask(index){
   ...
   changeStatus(index){
     ...
     if(++newIndex > 2 ){
      ...
      db.collection('tasks').doc(index).update({status:this.availableStatuses[newIndex]});
   }
  },
  firestore:{
